...
function obj = Hookean(varargin) 
    ...
    obj.params.Zeta   = 0.45;

    obj.contact.NormalDamping   = 0.05;
    obj.contact.NormalReaction  = 0.5;
    obj.contact.TangentReaction = 0.5;
    obj.contact.ContactFriction = 0.3;
...
%---------------------------------------------------------------------- get     
function y = getModulus(Hookean)
y = Hookean.params.E;
end
%---------------------------------------------------------------------- get     
function y = getContactReaction(Hookean)
y = Hookean.contact.NormalReaction*...
    getModulus(Hookean);
end
%---------------------------------------------------------------------- get     
function y = getContactTangentReaction(Hookean)
    y = Hookean.contact.TangentReaction*...
        getModulus(Hookean);
    end
%---------------------------------------------------------------------- get     
function y = getContactFriction(Hookean)
y = Hookean.contact.TangentFriction;
end
%---------------------------------------------------------------------- get     
function y = getContactDamping(Hookean)
y = Hookean.contact.NormalDamping...
    *getModulus(Hookean);
end
%---------------------------------------------------------------------- get    
function [S,S11,S33] = uniaxial(HookeanMaterial,x)
S   = zeros(numel(x),1);
S11 = zeros(numel(x),1);
S33 = zeros(numel(x),1);
for ii = 1:numel(x)
    F   = diag([x(ii),sqrt(1/x(ii)),sqrt(1/x(ii))]);
    PK2 = PiollaStress(HookeanMaterial,F);
    SIG = (1/det(F))*F*PK2*(F.');
...
%---------------------------------------------------------------------- set
function Hookean = set(Hookean,varargin)
    for ii = 1:2:length(varargin)
        Hookean.(varargin{ii}) = varargin{ii+1};
    end
end
%------------------------------ 2nd Piolla stress tensor for Neo-Hookean
function [S, D, P] = PiollaStress(Hookean,F,~)
%Se = 2nd PK stress [S11, S22, S33, S12, S23, S13];
C100 = Hookean.params.Mu/2; 
K    = Hookean.params.Lambda/2;
